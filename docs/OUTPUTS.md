...
|---|---|
| `merged_ledger.csv` | **The result.** Every merged peak (one row each): role, neutral formula + adduct, scores, ppm, confidence, tier, provenance, and the winning row's `admitted_by` / `occurrence`. The provenance anchor. |

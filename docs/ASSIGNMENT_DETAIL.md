...
`activate(*texts)` is the one call every entry point makes — `resolve_context_tags` then `active_lists`, returning `(lists, tags)` so the caller can log what the metadata unlocked. `assign_batch.run` calls it once with the batch name + reagent label and passes `reflists_active` to every per-sample `assign.run`; `cli.cmd_assign` and the MCP `assign_sample` tool call it with the run's context label. A single sample therefore has no batch name to read, so it activates the always-active lists (the contaminants) and nothing chemistry-specific unless its context label names one — enabling 1 + 2 on every path, and putting the same `reflists_active` versions in the single-sample manifest that `batch_summary.json` carries. The report (3) re-resolves the lists at build time through the same `activate`.

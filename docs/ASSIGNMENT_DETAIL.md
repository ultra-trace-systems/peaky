...
### 3.0b Source-solvent clusters (`solvent_clusters.assign_solvent_clusters`, pipeline stage `solvent_clusters`)

**Purpose**: claim the non-covalent ladders a positive source builds out of its own solvent vapour, and keep them **off** the neutral grid. Runs in pass 0's slot (straight after `pass0`, before `pass1`), **offline** — the ion masses are exact by construction and the evidence is the ladder, so no scorer is called; one could not be, since the neutral it would have to be handed does not exist.

**Why it is a separate channel.** `[(C2H6O)2+H]+` (m/z 93.0911) implies the neutral C4H12O2, whose DBE is −1: `chemistry.dbe_ok` refuses it, and that refusal is correct — the ion is two intact ethanols sharing a proton, not a molecule. On one EasyIC⁺ acquisition that single ion carried **6.5 % of total signal and about half of everything left in `role=unexplained`**, with its ¹³C satellite and the mixed `EtOH·H3O+` (65.0597) unexplained beside it. Loosening the DBE gate to reach them would open the whole DBE<0 corner of formula space to every other peak, so the family is enumerated explicitly instead — the reagent-cluster (`reagents.py`) and pass-0 known-species precedent.

**What is enumerated** (`build_library`), per source solvent S drawn from `ContextProfile.source_solvents` (`easyic`, `uronium`, `ammonium-15n` declare water / methanol / ethanol / acetone; every other context declares none and the stage is a no-op):

| ladder | ion | example |
|---|---|---|
| proton-bound | `[Sₙ + H]⁺`, n = 2…`MAX_UNITS` (4) | `(EtOH)₂H⁺` 93.0911, `EtOH·H₃O⁺` 65.0597 |
| hydride-bound | `[Sₙ − H]⁺` — the `[M-H]+` hydride channel *solvated*; only where the context HAS that channel (`carriers_for`), i.e. EasyIC, not the protonating urea/ammonium reagents | `[EtOH−H]⁺·EtOH` 91.0754 |
| condensation | either ladder's n = 2 rung **− H₂O** | 91.0754 − H₂O = 73.0648 (C₄H₉O⁺); 93.0911 − H₂O = 75.0805 (protonated diethyl ether) |

Mixed-solvent multisets are enumerated too; those sharing one ion composition (2×ethanol vs acetone+methanol) are **collapsed to the best-evidenced reading** and the rest recorded as `aliases`. An ion that IS a bare monomer channel is never enumerated — a water rung that then loses water is the molecule it started from (`[(C3H6O)(H2O)−H−H2O]⁺` is exactly `[C3H6O−H]⁺`), and claiming it would steal the anchor the family depends on. Water is capped at one per cluster (`MAX_WATER`) because it is the one **unanchored** solvent.

**Gates** (all must hold, per rung):

| Gate | Value |
|---|---|
| Monomer evidence | every non-water constituent shows its own `[S+H]⁺` / `[S−H]⁺` above the height gate, together ≥ `MIN_MONOMER_FRAC` (0.2 %) of the spectrum's total peak height. Water is exempt: H₃O⁺ (19.018) is below every window these sources acquire |
| Contiguous ladder | the rung below — the cluster minus one solvent, or for a condensation rung the un-dehydrated cluster **this run committed** — must be an **observed peak**, and the observed spacing must be that solvent's exact neutral mass within `LADDER_PPM` (4 ppm) |
| Mass | `\|ppm − cfg.prior_offset\| ≤ MASS_PPM` (2.0), the pass-0 gate |
| Intensity sanity | rung ≤ `MAX_RUNG_X_MONOMER` (3×) the monomers it is built from; a condensation rung ≤ `MAX_CONDENSATION_RATIO` (5×) its own parent cluster |
| Displacement floor | a rung whose composition **also** reads as a legal covalent neutral must additionally be ≥ `ALIAS_MIN_RUNG_FRAC` (0.1 %) of total signal — overruling a nameable molecule is a stronger claim than explaining an unexplained ion |
| No displacement by a WATER rung | a rung containing water may **never** displace a covalent reading, at any intensity. Every other unit is anchored by its own monomer ion *and* steps by a rare mass difference (on an orange-peel headspace spectrum 64 of 833 peaks, 7.7 %, had a +C2H6O partner within 4 ppm); water is anchored by nothing, so the whole claim is its 18.0106 step — and 218 of those same 833 peaks, **26 %**, had a +H2O partner. Unguarded it cost 11 of 18 files an Assigned `C3H6O2 [M+H]+`, displaced by an acetone-water cluster whose "monomer anchor" at 57.0335 was itself committed as protonated acrolein. A water rung with **no** covalent reading is still welcome — `EtOH·H3O+` (65.0597) implies a DBE −1 neutral, so there is no molecule to lose |

**What is committed.** The **meaningful neutral is the solvent** (the ammonia-via-urea-adduct and easyic-hydride rulings): `neutral_formula = C2H6O`, `adduct = [M+(C2H6O)H]+`, `ion_formula = C4H13O2+`. Repeated units are written out (`[M+(C2H6O)(C2H6O)H]+`) because that is the notation `tiers._ion_counts` parses; every label's shift is registered with `chemistry.register_adduct`, so `ion_mz(core, adduct)` reproduces the rung's mass from the committed row. All commits are **locked**, which is the point of running before pass 1.

Two evidence classes, tiered differently (`tiers.compute_tiers`):

- **no covalent reading exists** (the implied neutral's DBE is negative — every saturated-alcohol / water cluster): method `known:solvent_cluster`, **Assigned**, reason *"source-solvent cluster ion with no possible covalent reading"*.
- **a covalent reading exists**: method `cluster:solvent`, **Candidate**, the covalent readings stored in `alternatives`. They are derived from the context's own `reagent_adducts` (`_covalent_readings`), not a fixed pair, because which readings compete is the source's property and it grows — `C4H11O2+` is protonated C4H10O2 (a butanediol), and `C4H9O+` is protonated C4H8O, hydride-abstracted C4H10O *and*, since EasyIC gained `[M-CH3]+`, the methyl loss of pentanol. A reagent-cluster channel in that list (`[M+NH4]+`, the uronium adduct) drops out without being special-cased: it would need the reagent's own atoms in the ion. The cluster reading still wins the row — a same-ion covalent/cluster split goes to the adduct reading, the policy `tiers._drop_decomposition_aliases` already applies — but MS1 cannot prove it, so the tier says so.

Satellites need no special handling: `complete_isotope_envelopes` predicts from the committed `ion_formula`, so the dimer's ¹³C line (94.0944, measured 4.9 % against 4.4 % predicted for 4 C) is attached like any other.

Measured behaviour: on the ethanol-soaked acquisition 5 rungs, **10.1 % of total signal**, including the 6.5 % dimer and re-reads of 91.0754 (was protonated butanediol), 73.0648 (was C₄H₈O) and 75.0805 (was protonated butanol). Swept over **232 other EasyIC and urea per-file ledgers** it commits **9 rungs in total** — the ethanol dimer and its water loss in four headspace files, and one acetone dimer against an 8 %-of-signal acetone monomer. On every urea batch measured it commits **nothing**, because those windows start above ethanol's monomer ions and the family refuses a ladder whose monomer it cannot see; on the orange-peel headspace batches (49 files across three reagents) it likewise commits nothing. Regression: `tests/test_solvent_clusters.py`.
...
### 3.7a EasyIC⁺ fragmentation / cluster ambiguity (`annotate_easyic_ambiguity`, pipeline stage `easyic_ambiguity`)

**Gated on the easyic context**; runs among the post-tier stages. A low-pressure charge-transfer source fragments and clusters, so four MS1-irreducible readings recur. (1) A `CnH2n [M+H]+` alkene commit is the same ion as an alcohol's in-source dehydration `[CnH2n+2O + H − H2O]+`; when the alcohol is independently committed on its own hydride channel — or, with a batch time series, the pair correlates at `r ≥ ts_r_min` — the row is **relabelled** onto the alcohol (Assigned → Candidate), otherwise it keeps the alkene and carries the note. (2) Any heteroatom-free `CxHyOz [M+H]+` is ion-identical to `[CxH(y+2)Oz − H]+`: protonated carbonyl vs hydride-abstracted alcohol (59.049 was acetone **and** propanol on the 2026-08-31 gin run). Commentary only. (3) A pure-hydrocarbon cation with DBE ≥ 2 may be a fragment of a larger analyte. Commentary only. (4) **Cluster vs covalent** (`solvent_clusters.covalent_alias_notes`): the ion is also a source-solvent cluster of a solvent whose own monomer is strong in the same spectrum — `C4H11O2+` is both protonated butanediol and `[EtOH−H]⁺·EtOH`, `C4H9O+` both protonated C₄H₈O and that dimer's water loss. §3.0b **commits** such an ion as the cluster when the whole ladder is behind it; this note is for the rows it did not take (ladder broken, rung below the displacement floor, or the channel off), so the covalent reading stands with the cluster reading recorded beside it.

Cases 1–3 skip **locked** rows; case 4 does not — a lock says no pass may *change* the reading, but the pass that locked a High grid winner never weighed a cluster reading at all, and `C4H11O2+`/`C4H9O+` are locked pass-1 backbone rows on the run this came from. Case 4 never changes a reading either way. Regression: `tests/test_cleanup.py`, `tests/test_solvent_clusters.py`.
...
3. Per cluster the files VOTE in two stages (`_vote`). Which ION (`_ion_key` = neutral + adduct composition): a curated neutral (reflist-rescue / pass-0 known-species provenance, `_curated_neutrals`) Assigned in at least one file and in no fewer files than any grid ion wins outright (noted in the merged `tier_reason`); otherwise the ion carried by the most files, then the most files at Assigned (`TIER_RANK = {Assigned:2, Candidate:1}`), then the best `ion_score`, then the ion's text (a deterministic last key). Which LABEL of that ion (the reagent-N isobar, e.g. `C13H14O4 [M+NH4]+` vs `C13H17NO4 [M+H]+`): the reading Assigned in the most files — on such a pair Assigned means a discriminating channel was present and Candidate means the file could not tell — then file count, then score (noted in `tier_reason` when it overrides a bigger count). The winner's best per-file row supplies tier / score / provenance; consensus `mz` = mean of raw; `n_files`, `n_files_ion`, `n_files_winner`, `alternatives` (the losers, `formula adduct xN tier score`); `mz_jitter_ppm_raw` and `mz_jitter_ppm_caldj` (offset-corrected residual); `ion_agree` (one ion in the cluster) and `formula_agree` (True if all files report the same neutral — different adducts of one neutral still agree). See [`MERGE.md`](MERGE.md) §3 for why a vote replaced the Assigned-count-first rule, and why it has two stages.

**Polarity-aware cleanup, once per batch**: for positive uronium, `cleanup.relabel_reagent_n_adducts(merged)` (the per-file stage is off: `assign_kw["reagent_n_relabel"] = False`) and then `cleanup.prefer_amine_over_ammonium(merged, ts_peaks, r_min=0.6)`, both at the merged level; each writes its note to the merged row's `tier_reason`, and `batch_summary.json["merge_gates"]` keeps their counts.
...
`load_catalog(directory)` loads packaged `*.json` peaklists into `{id: ReferenceList}` (self-describing: id/system/label/data_version/references/provenance/`always_active`/`applies_to_contexts`, `species[{formula, conditions, radical}]`; radical status is read off the formula's DBE parity, the flag is a claim checked against it). `resolve_context_tags(*texts)` infers experimental-context tags from run metadata (batch name / label) via `CONTEXT_KEYWORDS` — the "unlock with metadata" step. `active_lists(catalog, context_tags)` returns `always_active` lists (e.g. lab contaminants) + any whose `applies_to_contexts` intersect the tags (polarity NOT filtered — neutral formulas transfer across reagent ion forms). Current lists: `monoterpene_hom_kang2024` (830 HOM, 257 of them radicals, context-gated) + `contaminants_keller2008` (50 organic neutrals, always-active).
...
1. **Selection prior** (`arbitrate`, §2.5/§4.1) — a candidate neutral on an active list gets a `+0.04` tie-break in `eff_score`. `assign.run` sets `cfg.reflist_formulas` from `reflists_active` (`reflists.prior_formulas` — the active lists' closed-shell pools, radicals excluded) and records `reflists_active = [(id, data_version), ...]` (`reflists.active_versions`) in its result, so the sample manifest (and `batch_summary.json`) says which list versions shaped the prior and the rescue. Empty when the caller activated nothing.
...
`activate(*texts)` is the one call every entry point makes — `resolve_context_tags` then `active_lists`, returning `(lists, tags)` so the caller can log what the metadata unlocked. All three hand it the same kinds of run metadata, so one sample resolves the same tags whichever one started it: `assign_batch.run` calls it once with the batch name + reagent label and passes `reflists_active` to every per-sample `assign.run`; `cli.cmd_assign` and the MCP `assign_sample` tool call it with the run's context + reagent label. A single sample therefore has no batch name to read, so it activates the always-active lists (the contaminants) and nothing chemistry-specific unless its context or its reagent label names one — enabling 1 + 2 on every path, and putting the same `reflists_active` versions in the single-sample manifest that `batch_summary.json` carries. The report (3) re-resolves the lists at build time through the same `activate`.

...
3. Per cluster the files VOTE on the `(neutral_formula, adduct)` reading (`_vote`): a curated neutral (reflist-rescue / pass-0 known-species provenance, `_curated_neutrals`) Assigned in at least one file and in no fewer files than any grid reading wins outright (noted in the merged `tier_reason`); otherwise the reading carried by the most files, then the most files at Assigned (`TIER_RANK = {Assigned:2, Candidate:1}`), then the best `ion_score`, then the reading's text (a deterministic last key). The winner's best per-file row supplies tier / score / provenance; consensus `mz` = mean of raw; `n_files`, `n_files_winner`, `alternatives` (the losers, `formula adduct xN tier score`); `mz_jitter_ppm_raw` and `mz_jitter_ppm_caldj` (offset-corrected residual); `formula_agree` (True if all files report the same neutral — different adducts of one neutral still agree). See [`MERGE.md`](MERGE.md) §3 for why a vote replaced the Assigned-count-first rule.

...
| **0s**       | `solvent_clusters` | **source-solvent clusters** (positive mode, committed + locked, in pass 0's slot, offline — no scorer): the proton-/hydride-bound ladders a solvent-vapour source builds out of itself, `[Sₙ+H]⁺` / `[Sₙ−H]⁺` and their `−H₂O` condensation rung. Deliberately OFF the neutral grid — the ethanol dimer's implied neutral C₄H₁₂O₂ has DBE −1, and that gate is right. Gated on the solvent's own monomer ion (present, ≥0.2 % of signal), an observed rung below at exact spacing, and the pass-0 mass gate; a rung that would displace a legal covalent reading must also be a major ion and is capped at Candidate with the covalent reading stored. Opened per context via `ContextProfile.source_solvents` |
...
| cleanup      | `cleanup`         | isotope-confirmed recovery, bromide-cluster labelling, ringing/sidelobe artifact flagging; **plausibility demotes** (carbon-cluster / implausible-ionization / speculative-residual, post-tier) + reagent-halocarbon relabel (Br runs) + **positive-mode reagent-N re-read** (a pure hydrocarbon via an NH₄/urea cluster → `[M+H]⁺` of an N-heterocycle `M+(cluster−H)`, Ur runs) + **`nh4_dehydration`** (labelled-ammonium runs): the in-source declustering cascade `[M+^NH4]⁺ → [M+H]⁺ → [M+H−H₂O]⁺` and `[M+^NH4]⁺ → [M+^NH4−H₂O]⁺` — an alkene/alkyne reading sitting on a hydrate's water-loss ion is re-read as the parent's dehydration alias (relabel-only, so such rows are capped at Candidate), a second water loss is annotated only; + **`easyic_ambiguity`** (EasyIC runs): the carbonyl-vs-alcohol, fragment-vs-intact and **cluster-vs-covalent** dual readings — the last one the complement of the `0s` stage, stamped on the rows that stage could not take |

...
| 0s            | **source-solvent clusters** (positive mode, committed + locked, straight after pass 0): the proton- and hydride-bound ladders a solvent-vapour source builds out of itself — `[Sₙ+H]⁺`, `[Sₙ−H]⁺` and their `−H₂O` condensation rung, over ethanol/methanol/acetone/water. These are **unreachable by the neutral grid on purpose** (the ethanol dimer's implied neutral C₄H₁₂O₂ has DBE −1), and on one EasyIC⁺ acquisition `(C₂H₅OH)₂H⁺` alone was **6.5 % of total signal** and half the unexplained residual. A rung commits only with its solvent's own **monomer ion present and ≥0.2 % of signal**, the **rung below observed** and the spacing exact; a rung that would displace a legal covalent reading (C₄H₁₁O₂⁺ is also protonated butanediol) must additionally be a major ion, and may never be a **water** rung (water is anchored by nothing and +18.0106 is the commonest mass difference in a spectrum); a rung that does displace is tiered **Candidate** with the covalent reading recorded. Opened per context (`ContextProfile.source_solvents`); inert wherever the solvent monomers are absent or out of window |
